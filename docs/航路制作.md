...
## routetable.ini
默认放在```ATC4\PORT\RXXX\SCENARIO```内如果做出修改，请和command 和state 一样放入```SCENARIO\0015_xxxx_ Stage```下，避免影响所有关卡
```ini
[AREA_A] 
;DEP SOUJA ARR ATMUG 
LTFM=Istanbul 
...
RJSS=Sendai
```
> 让如果你知道ini格式的标准，后面的你基本可以不用看
/[AREA_?]  区域编号，给机场进行分区
```ini
;注释
```
机场的ICAO=机场名字```ICAO=name```
```ini
[DEP_ROUTE] 
GP_DP06L_SOUJA=RWY06L,DP_RINKU_1_DEP_RWY06L,TR_MAIKO_SOUJA_TRANS 
GP_DP06L_OLIVE=RWY06L,DP_IWAYA_3_DEP_RWY06L,TR_MAIKO_OLIVE_TRANS,DM_OLI
VE_V38 
...
```
[DEP_ROUTE]离场路线
线路组合名字=线路1，线路2，...（填ROUTE 里边.ard 文件的名称） 组合名=route1,route2
```ini
[DEP_TABLE_A] 
comment=Day 
time=0000-2359 
...
AREA_C=GP_DP06L_GUJYO/GP_DP06R_GUJYO/GP_DP24L_GUJYO/GP_DP24R_GUJYO
```
[DEP_TABLE_?]离场表，？部分从A 往后填，有几种运行情况就填机种 
```ini
comment=注释 
time=时间（hhmm-hhmm） 
wind=风向 
...
runway=所有跑道 
active=启用跑道 
AREA_?=从跑道1 起飞的线路组合名/从跑道2 起飞的线路组合名/...
```
```ini
[APP_ROUTE] 
...
```ini
[APP_TIME] 
Not sure what is the use of it, it can be blank  不确定干啥的，可能是影响时刻表关卡飞机的出生时间
的，可以为空 
...
AREA_A=GP_AP34L_GODIN2K/GP_AP34R_GODIN1H/GP_AP22_GODIN1D/GP_AP23_GODIN1D
/GP_AP16L_GODIN_L_HIGH/GP_AP16R_GODIN_R_HIGH 
AREA_B=GP_AP34L_AROSA1K_SOUTH/GP_AP34R_AROSA1A_SOUTH/GP_AP22_AROSA1B_S
OUTH/GP_AP23_AROSA1B_SOUTH/GP_AP16L_AROSA_L_SOUTH_HIGH/GP_AP16R_AROSA_
R_SOUTH_HIGH 
AREA_C=GP_AP34L_GODIN2K/GP_AP34R_GODIN1H/GP_AP22_GODIN1D/GP_AP23_GODIN1D
/GP_AP16L_GODIN_L_HIGH/GP_AP16R_GODIN_R_HIGH 
...
runway=RWY34L,RWY34R,RWY22,RWY23,RWY16L,RWY16R 
active=RWY34L,RWY34R,RWY22,RWY23,RWY16L,RWY16R 
AREA_A=GP_AP34L_GODIN1C/GP_AP34R_GODIN1C/GP_AP22_GODIN1S/GP_AP23_GODIN1S/
GP_AP16L_GODIN_L_LOW/GP_AP16R_GODIN_R_LOW 
AREA_B=GP_AP34L_AROSA1A_SOUTH/GP_AP34R_AROSA1A_SOUTH/GP_AP22_AROSA1N_S
OUTH/GP_AP23_AROSA1N_SOUTH/GP_AP16L_AROSA_L_SOUTH_LOW/GP_AP16R_AROSA_
R_SOUTH_LOW 
AREA_C=GP_AP34L_GODIN1C/GP_AP34R_GODIN1C/GP_AP22_GODIN1S/GP_AP23_GODIN1S/
GP_AP16L_GODIN_L_LOW/GP_AP16R_GODIN_R_LOW
```
[APP_TABLE_?]进场表
comment=注释 
...
runway=所有跑道 
active=启用跑道 
AREA_?=降落跑道1 线路组合名/降落跑道2 线路组合名/...
## fix.ini
文件位于```ATC4\PORT\RXXX\RADAR```
```ini
[FIX] 
...
```
航点名称,经度（度：分：秒），纬度（度，分，秒），航点类型**航点数量是有上限的（不清楚具体多少，关西大概是190 个左右）**该文件用于设置飞机的出生点位，使用关卡编辑器时，在空中出生位置中填入航点名称即可（如果填fix.ini 里没有的航点，飞机将直接刷在0，0 点）